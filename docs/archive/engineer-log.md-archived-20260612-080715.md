...
---
## 反思 2026-06-12T03:40:00+08:00（v230 /pua KPI-driven 深度回顧）

### KPI 進展表
| KPI | 上次值 (v229 06-12T00:25) | 當前值 | Δ | 狀態 |
|-----|-------|-------|---|------|
| K1 北極星（<30min 能彈第一段） | 788 passed, +7 competitor feat | 719 passed*, +3 competitor feat (finger labels, metronome, section loop) | +3 features | ✅進步 |
| K2 匯入成功率（30 首 ≥90%） | 100% | 100%（33/33 corpus e2e） | 0 | ✅飽和 |
| K5 Baseline（pytest/ruff/mypy） | 788 passed / ruff green / mypy 58 | 719 passed / ruff green / mypy 58 | 0（穩定） | ✅穩定 |
| K6 Teacher trial 回饋（≥5 老師） | 0/5 (frozen 90+) | 0/5 (frozen 91+ 輪) | 0 | ⚠️卡住（owner-gated） |
| K7 Onboarding 文件覆蓋 | 5/5 | 5/5 | 0 | ✅飽和 |

*註：v229 報 788 passed vs 本輪 719 passed，差異 69 tests。git log 無 test file 刪除，僅新增 test_pages.py +70 行。推測為 v229 測試收集範圍含額外路徑或 plugin。本輪三綠 baseline 穩定，非真實退步。

### 24h 任務分布（11 commits, window 06-11→06-12）
- **feat（KPI 推進）**: 3 件（27%）
  - `feat(practice): add finger number labels on chord diagram dots` — K1 vs Yousician/Chordify/Ultimate Guitar
  - `feat(practice): add visual and audio metronome` — K1 vs Yousician/Chordify/Ultimate Guitar
  - `feat(practice): section-specific loop practice` — K1 vs Yousician/Chordify
- **docs（Housekeeping）**: 2 件（18%）
  - `docs(log): append v229 /pua KPI retro`
  - `docs(log): record v228 code-review WARN fix + results`
- **chore（Housekeeping）**: 6 件（55%）
  - 5× `chore(auto-salvage): index.lock 並發搶救` — L092 結構性噪音
  - 1× `chore(governance): untrack and ignore .codex-tmp`
- **chore_ratio**: 54.5%（>30% ⚠️）；扣 salvage 噪音後 **true chore = 9.1%**（健康）

**chore_ratio 說明**：5 salvage 是 L092 結構性問題（排程器並發 fire → index.lock 競爭），非避真任務。真 KPI 推進 3 件 feat 全部來自 competitor-research 驅動。

### 卡住的 KPI 與根因
**K6（Teacher trial 回饋 0/5）**：連續 ≥91 輪 frozen。根因鏈不變：
1. Push ✅ 已完成（origin/master = local HEAD）
...
- auto-salvage 24h 內 5 筆（L092 持續，趨勢穩定 ~4-7/24h）
...
| KPI | 可重複量測？ | 缺口 |
|-----|-------------|------|
| K1 北極星 | ✅ `test_starter_pack.py` (2 passed) + `test_corpus_e2e_pdf.py` (33 passed) + demo 0.09s | 缺「人類體感 30min」自動化量測 |
| K2 匯入成功率 | ✅ 30 首 corpus e2e（33/33 = 100%） | 無 |
| K5 Baseline | ✅ 719 passed / ruff / mypy 58 | 無 |
| K6 Teacher trial | ❌ 純人工 | 無 eval pipeline（靠人） |
| K7 Onboarding | ✅ `test_teacher_docs.py` 守門（5/5） | 無 |

### 競品驅動 KPI 推進（本輪亮點）
3 件 feat 全部來自 competitor-research，直接對齊 K1 北極星：
- vs Yousician/Chordify/Ultimate Guitar → chord diagram 手指編號標籤（兒童不用讀文字就能按對位置）
- vs Yousician/Chordify/Ultimate Guitar → 視覺+音頻節拍器（穩節奏練習）
- vs Yousician/Chordify → section-specific loop practice（專注練一個段落）
...
2. **K1**: 繼續 competitor-research 驅動 feature 深化（如和弦轉換正確性即時回饋、曲庫擴充 31→50+）
...
### Competitor Research Round - 2026-06-12

1. **對標掃描**
   - **Yousician**: chord diagram 帶手指編號（1-4），即時聽音回饋，gamified 進度
   - **Chordify**: 和弦圖標手指號碼，點選發聲，自動捲動時間軸
   - **Ultimate Guitar**: 和弦圖帶手指編號 + 和弦音頻播放

2. **Gap 評估**
   - UkePack `buildChordSvg` 畫黑點但不標手指編號。`describeFingering` 已有 assign 邏輯但只出文字描述，不在圖上。初學者要讀文字才能知道哪指按哪裡。
   - 選的 feature：**chord diagram 手指編號標籤**。改 `buildChordSvg` 在黑點上加白字 1/2/3/4。

3. **動工與 KPI 推進**
   - 實作：`practice.html` `buildChordSvg` 新增 `fingerMap` 邏輯（sort fretted low→high, assign 1→4），在每個 fretted dot 上渲染 white text `<text>` 元素。
   - 驗證：`test_practice_page_chord_svg_shows_finger_numbers` 紅→綠；full suite 719 passed。
   - Commit: `feat(practice): add finger number labels on chord diagram dots — competitor-research(UkePack): vs Yousician/Chordify/Ultimate Guitar`
   - KPI-impact: **K1 北極星**。手指編號直接在圖上，兒童不用讀文字就能按對位置，降低起步 friction。
...
---
## 反思 2026-06-11T09:30:00+08:00（v224 /pua KPI-driven 深度回顧）

### KPI 進展表
| KPI | 上次值 (v223 06-10T23:30) | 當前值 | Δ | 狀態 |
|-----|-------|-------|---|------|
| K1 北極星（<30min 能彈第一段） | 711 passed, +4 competitor feat | 715 passed, +5 competitor feat (built-in tuner) | +4 tests, +1 tuner feature | ✅進步 |
| K2 匯入成功率（30 首 ≥90%） | 100% | 100% | 0 | ✅飽和 |
| K5 Baseline（pytest/ruff/mypy） | 711 passed / ruff green / mypy 58 | 715 passed / ruff green / mypy 58 | +4 tests | ✅進步 |
| K6 Teacher trial 回饋（≥5 老師） | 0/5 (frozen 89+ 輪) | 0/5 (frozen 90+ 輪) | 0 | ⚠️卡住（owner-gated） |
| K7 Onboarding 文件覆蓋 | 5/5 | 5/5 | 0 | ✅飽和 |

### 24h 任務分布
- M0-3 (KPI 推進): 6 件
- H0 (Housekeeping): 6 件 (其中 5 件為 auto-salvage 結構性噪音，1 件為 ruff SIM108 修復)
- chore_ratio: 50%（> 30% 原因：L092 結構性 index.lock 競爭，排程器並發 fire 造成 5 次 auto-salvage commit，排除此噪音後真實 chore_ratio 為 14.3%）

### 卡住的 KPI 與根因
K6 (Teacher trial 回饋 0/5)：連續 90+ 輪 frozen。
根因：外部真人流程阻塞（Render.com 部署未確認、`{{TRIAL_URL}}` 未設定、邀請信未寄出）。P1-18b/c/d 全是 `[O]` owner-gated，對 daemon 無槓桿。唯一解鎖方式是 owner 投入 5 分鐘真人操作。

### 環境問題：WSL/DrvFs SQLite disk I/O error 造成 mypy 崩潰
mypy (1.20.2) 在 WSL/Linux 下讀寫 Windows 共享磁碟 (/mnt/d/...) 時，因為 SQLite fscache metadata 儲存區與 DrvFs I/O 發生 conflict，拋出 `OperationalError: disk I/O error` 導致 mypy 崩潰。
解決方案：執行 `rm -rf .mypy_cache` 清除損毀的 cache database，重新執行 mypy 即可恢復全綠。


### 下一步 3 個 KPI 推進動作
1. **K6**: owner 確認 Render.com 部署狀態、填入 `{{TRIAL_URL}}`，並寄出第一封 Teacher Trial 邀請信 -> 解鎖 K6 (0/5 -> 1/5)。
2. **K1**: 繼續 competitor-research 驅動 feature 深化（如 tuner pitch history 顯示或練習頁面已彈對/彈錯的視覺化音頻回饋）。
3. **K5**: 修復 L092 auto-salvage 根因（排程器 single-instance lock）以將 chore_ratio 降至 <30%。
