# UkePack 老師試用問卷

**試用日期**：___________________  
**老師姓名／代號**：___________________  
**測試曲目**：___________________  
**使用難度等級**：___________________  
**試用形式**：直播帶看 / 老師自行操作 / 先看影片後回填  
**裝置 / 瀏覽器**：___________________  
**從開始到第一份 PDF 用時**：___________________

---

## 量測 Metadata

> 對應 `docs/teacher/polaris_measurement.md` 量測步驟；試用當下填入，驗證北極星 < 30 分鐘。

**Packet 寄出 / 分享 Timestamp**：___________________  
**老師打開 Web UI Timestamp**：___________________  
**第一份 PDF 完成 Timestamp**：___________________  
**學生試彈第一段 Timestamp**：___________________  
**總用時（步驟 1 → 4）**：___________________ （目標 < 30 分鐘）  
**是否達標**：是 / 否

---

## 主持人觀察紀錄（開發者填）

> 試用當下就填。不要等結束後憑印象回想。

**第一次卡住的步驟**：

---

**卡住超過 5 秒的原話 / 行為**：

- 
- 
- 

**老師主動稱讚的點**：

- 
- 

**是否願意直接拿去上課**：可以 / 需要小修 / 不行

**如果需要小修，缺的是什麼**：

---

## 5 題反饋清單

> 每題選一個選項，並在「補充說明」填寫具體情況（可省略）。

### Q1 — 難度分級準確度

系統給出的 Level 1 / 2 / 3 標示，符合我對學生實際難度的預期。

- [ ] 非常同意
- [ ] 同意
- [ ] 普通
- [ ] 不同意
- [ ] 非常不同意

**補充說明**：

---

### Q2 — 字體大小與可讀性

PDF 中的字體大小（和弦名稱、段落標題、練習說明）在教室上課或一對一課時看得清楚。

- [ ] 非常同意
- [ ] 同意
- [ ] 普通
- [ ] 不同意
- [ ] 非常不同意

**補充說明**：

---

### Q3 — 和弦圖清晰度

GCEA 和弦圖（手指位置、空弦、把位格數）夠清楚，初學學生不需要老師額外說明就能看懂。

- [ ] 非常同意
- [ ] 同意
- [ ] 普通
- [ ] 不同意
- [ ] 非常不同意

**補充說明**：

---

### Q4 — 刷法建議合理度

系統建議的刷法符合這首歌的風格與速度，我可以直接拿來教。

- [ ] 非常同意
- [ ] 同意
- [ ] 普通
- [ ] 不同意
- [ ] 非常不同意

**補充說明**：

---

### Q5 — 整體可用性

這份 PDF 練習包我可以直接用在實際的烏克麗麗課程，不需要再自行修改。

- [ ] 非常同意
- [ ] 同意
- [ ] 普通
- [ ] 不同意
- [ ] 非常不同意

**補充說明**：

---

## 開放式問題

**如果只能改一件事，您會優先改什麼？**

---

**其他觀察或建議？**

---

## Conclusion（Step 18d 完成後填）

### 1. 最常見的 3 個卡點

1. 
2. 
3. 

### 2. 下一輪最該改的 1 件事

- 

### 3. 落地動作

- [ ] 轉成 backlog item
- [ ] 標成 known issue
- [ ] 只記錄，暫不處理

*感謝您撥出時間試用！您的回饋將直接影響 UkePack 下一版的開發優先順序。*
