"""U6-a starter pack coverage: 143 public-domain songs end-to-end to PDF."""
...
def _load_starter_pack_song_paths() -> list[Path]:
    ...
    assert len(paths) == 143, f"Starter pack must contain 143 songs, got {len(paths)}."
    return paths
...
def test_starter_pack_has_143_public_domain_songs() -> None:
...
def test_starter_pack_render_p95(tmp_path: Path) -> None:
    """143-song corpus p95 render time must stay under the north-star gate."""
    timings: list[float] = []
    for path in _load_starter_pack_song_paths():
        output = tmp_path / f"{path.stem}.pdf"
        elapsed = run(path, 1, output, "public_domain")
